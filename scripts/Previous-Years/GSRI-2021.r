# Visualization and Analytics for 2021 GSRI Survey Respondents
# Joe Lollo, UW Libraries

# Import all relevant libraries (installation is once per machine):
...
# Load 2021 survey as CSV:
responses_df <- read.csv("https://raw.githubusercontent.com/ChessPiece21/GSRI-SurveyReview/main/data/GSRI-2021-Refined.csv", stringsAsFactors = FALSE)
...
studentPop_plot <- ggplot(studentPop_df) + 
  ...
  labs(title = "GSRI 2021 Survey Respondents, by Discipline and Field", x = "Discipline", y = "Number of Students", fill = "Area of Study")
...
session_plot <- ggplot(session_df) +
  ...
  labs(title = "GSRI 2021 Survey Respondents, by Session Attended", x = "Session", y = "Number of Students")

ggplotly(session_plot)

# Equal number of respondents per session, but this is only half of the enrolled grad students across the 2021 GSRI sessions.
...
slack_plot <- ggplot(slack_df) + geom_col(aes(x = Satisfied.Slack, y = count, fill = Session)) +
  labs(title = "GSRI 2021 Survey Respondents' Satisfaction with Slack", x = "Indicator", y = "Number of Students")

ggplotly(slack_plot)

canvas_plot <- ggplot(canvas_df) + geom_col(aes(x = Satisfied.Canvas, y = count, fill = Session)) +
  labs(title = "GSRI 2021 Survey Respondents' Satisfaction with Canvas", x = "Indicator", y = "Number of Students")
